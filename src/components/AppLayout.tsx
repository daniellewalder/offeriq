    ...
    <div className="min-h-screen flex bg-background">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-sidebar border-r border-sidebar-border flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="px-6 pt-7 pb-6 border-b border-sidebar-border">
          ...
          <p className="text-[9.5px] tracking-[0.22em] uppercase text-sidebar-foreground/45 mt-2 font-body font-medium">Offer Intelligence</p>
        </div>
        <nav className="flex-1 px-3 py-5 overflow-y-auto">
          {navSections.map((section, si) => (
            <div key={section.label} className={si > 0 ? 'mt-6' : ''}>
              <p className="px-3 mb-2 text-[9.5px] tracking-[0.22em] uppercase text-sidebar-foreground/40 font-body font-medium">{section.label}</p>
              <div className="space-y-px">
                {section.items.map((item) => {
                  const isActive = location.pathname === item.to;
                  return (
                    <Link key={item.to} to={item.to} onClick={() => setSidebarOpen(false)}
                      className={`relative flex items-center gap-3 px-3 py-[7px] rounded-md text-[12.5px] transition-all duration-200 font-body group ${isActive ? 'bg-sidebar-accent text-sidebar-accent-foreground font-medium' : 'text-sidebar-foreground hover:text-sidebar-accent-foreground hover:bg-sidebar-accent/40'}`}>
                      {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 h-4 w-[2px] bg-accent rounded-r" />}
                      <item.icon className={`w-[15px] h-[15px] flex-shrink-0 ${isActive ? 'text-accent' : 'text-sidebar-foreground/70 group-hover:text-sidebar-foreground'}`} strokeWidth={1.5} />
                      {item.label}
                    </Link>
                  );
                })}
        ...
        <div className="px-3 py-3 border-t border-sidebar-border">
          <div className="flex items-center gap-3 px-3 py-2 rounded-md">
            ...
            <button type="button" onClick={handleSignOut} className="text-sidebar-foreground/60 hover:text-sidebar-accent-foreground transition-colors p-1.5 rounded hover:bg-sidebar-accent/40" title="Sign out">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-border/70 flex items-center px-4 lg:px-12 bg-background/85 backdrop-blur-md sticky top-0 z-30">
          <button className="lg:hidden mr-3 p-1.5 rounded-md hover:bg-muted text-foreground" onClick={() => setSidebarOpen(true)}>
